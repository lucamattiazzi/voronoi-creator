...
export async function SPSA(
  ...
  const zeroCost = await costFunction(theta)
  console.log('zeroCost', zeroCost)
  while (true) {
    const gradients = await getGradients(costFunction, theta, ck.next().value, constraints)
    const nextAk = ak.next().value
    theta = theta.map((value, idx) => clamp(value - nextAk * gradients[idx], constraints))
    const currentCost = await costFunction(theta)
